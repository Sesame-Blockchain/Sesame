 ...
 * @notice It vests the Sesame tokens to an owner over a linear schedule.
...
contract VestingContractWithFeeSharing is Ownable, ReentrancyGuard {
     ...
     * @param _maxAmountToWithdraw maximum amount in Sesame to withdraw per period
    ...
    function unlockLooksRareToken() external nonReentrant onlyOwner {
        ...
        uint256 balanceToWithdraw = sesameToken.balanceOf(address(this));
        ...
        // Transfer Sesame to owner
        sesameToken.safeTransfer(msg.sender, balanceToWithdraw);
    ...
    {
        require(
            _currency != address(sesameToken),
            "Owner: Cannot withdraw Sesame"
        );
